...
public interface IFarmProcess extends IBaritoneProcess {
    ...
    /**
     * Begin to search for specific crops to farm within the specified
     * area from the specified location.
     *
     * @param range  The distance from center to farm from
     * @param pos    The center position to base the range from
     * @param filter The crops to farm
     */
    void farm(int range, BlockPos pos, BlockOptionalMetaLookup filter);

    /**
     * Begin to search for specific crops to farm.
     *
     * @param boms The crops to farm
     */
    default void farm(BlockOptionalMeta... boms) {
        farm(0, null, new BlockOptionalMetaLookup(boms));
    }
